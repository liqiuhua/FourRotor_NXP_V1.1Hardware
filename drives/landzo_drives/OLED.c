...
************************************************************************/
  ...
  #define OLED_SCL  PTE6_OUT
  #define OLED_SDA  PTE5_OUT
  #define OLED_RST  PTE4_OUT
  #define OLED_DC   PTE3_OUT
  #define OLED_CS   PTE2_OUT
...
{
    int8_t j=0,plus=0,tab=0,num[5]={0};
   // static int8_t length=0;
    ...
    if(plus){
      //length = tab;
      for(;tab>=0;tab--)    OLED_Num(x+(j-tab),y, num[tab]); }
    else  {  
      //length = tab+1;
      OLED_Showplus(x,y);
      for(;tab>=0;tab--)    OLED_Num(x+(j-tab+1),y, num[tab]); } 
}
