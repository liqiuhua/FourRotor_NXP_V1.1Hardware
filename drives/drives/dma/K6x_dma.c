                          ...
                          uint32 count,uint8 SOFF,uint8 DOFF, DMA_PORTx2BUFF_cfg cfg)
{
    ...
    DMA_SOFF(CHn)  =    SOFF;                              // 设置源地址偏移 = 0x0, 即不变
    DMA_DOFF(CHn)  =    DOFF;                              // 每次传输后，目的地址加 BYTEs
   ...
   // 配置 DMA 传输结束后的操作 
    ...
    {
      DMA_SLAST(CHn)      =   (uint32)( (cfg & 0x20) == 0 ? (-(count))  : 0 ); //调整目的地址的附加值,主循环结束后恢复目的地址或者保持地址
    }
    if(DOFF == 0)
    {
      DMA_DLAST_SGA(CHn)  = 0 ;
    }else{
      DMA_DLAST_SGA(CHn)  =   (uint32)( (cfg & 0x20) == 0 ? (-(count) ) : 0 ); //调整目的地址的附加值,主循环结束后恢复目的地址或者保持地址
    }
   ...
   //配置 DMA 触发源 
    DMAMUX_CHCFG_REG(DMAMUX_BASE_PTR, CHn) = (0
          ...
          //  | DMAMUX_CHCFG_TRIG_MASK                        // Trigger Mode: Periodic   PIT周期触发传输模式   通道1对应PIT1，必须使能PIT1，且配置相应的PIT定时触发 
            | DMAMUX_CHCFG_SOURCE(DMSour)                   // 通道触发传输源: 触发通道需要自己修改    
                                             );
...
void DMA_PORTx2BUFF_Init(DMA_CHn CHn, void *SADDR, void *DADDR, PTxn ptxn, DMA_BYTEn byten, uint32 count, DMA_PORTx2BUFF_cfg cfg)
{
   ...
   /* 配置 DMA 触发源 */
    ...
    {
    case DMA_BYTE1:
        *((uint8 *)((uint32)SADDR + 4)) = 0;   //设置为输入方向。为什么加4？PDIR地址加4后，就变成对应的PDDR地址
        break;
    case DMA_BYTE2:
    ...
    // 输入源管脚选择功能脚  
    pinMIN = (uint8)(((uint32)SADDR - ((uint32)(&PTA_BYTE0_INT))) & 0x3f);         //最小的引脚号
    tmp = pinMIN + (BYTEs << 3);                                         //最大的引脚号
...
void DMA_count_Init(DMA_CHn CHn, PTxn ptxn, uint32 count, DMA_Count_cfg cfg)
{
    ...
    /* 配置 DMA 通道 的 传输控制块 TCD ( Transfer Control Descriptor ) */
    DMA_SADDR(CHn) =    (uint32)COUNTSADDR;                    // 设置  源地址
    DMA_DADDR(CHn) =    (uint32)COUNTDADDR;                    // 设置目的地址
