...
//配置描述符
const uint8 Configuration_Descriptor[0x20]= 
{
	//配置描述符
...
//端点IN(2)状态
uint8 vEP2State = kUDATA1;
//端点OUT(3)状态
uint8 vEP3State = kUDATA0;

//保存端点OUT的数据
uint8 RecDataBuff[EP3_SIZE];
//数据长度
...
uint8 gu8Dummy;
uint8 gu8HALT_EP;
uint8 gu8USB_State;                //保存USB端点0的状态
...
//返  回: 无
//=========================================================================
void hw_usb_ep_in_transfer(uint8 u8EP,uint8 *pu8DataPointer,uint8 u8DataSize)
{
    uint8 *pu8EPBuffer;
    uint8 u8EPSize; 
    uint16 u16Lenght=0;    
...
//返  回: 接收到的数据长度
//=========================================================================
uint8 hw_usb_ep_out_transfer(uint8 u8EP,uint8 *pu8DataPointer)
{
    uint8 *pu8EPBuffer;
    uint8 u8EPSize; 
...
void hw_usb_stdReq_handler(void)
{
    ...
    switch(Setup_Pkt->bRequest)
    {
        ...
        */ 
          ...
          //  低字节 : Setup_Pkt->wValue_l 表示描述符的索引值
            switch(Setup_Pkt->wValue_h) 
            {
                ...
                */ 
                case mDEVICE:
                    // 发送设备描述符
                    hw_usb_ep_in_transfer(EP0,(uint8*)Device_Descriptor,sizeof(Device_Descriptor));
                    break;
                    
                case mCONFIGURATION:
                    // 发送配置描述符
                    hw_usb_ep_in_transfer(EP0,(uint8*)Configuration_Descriptor,sizeof(Configuration_Descriptor));
                    break;
        
                case mSTRING:
                    // 发送字符串描述符
                    hw_usb_ep_in_transfer(EP0,(uint8*)String_Table[Setup_Pkt->wValue_l],String_Table[Setup_Pkt->wValue_l][0]);
                    break;
        ...
        case mGET_CONFIG:
            // 获取配置信息
            hw_usb_ep_in_transfer(EP0,(uint8*)&gu8Dummy,1);
            break;
        // 获取状态 Get Status 
        // #define mGET_STATUS           0 
        case mGET_STATUS:
            // 
            gu8Status=0;
            hw_usb_ep_in_transfer(EP0,(uint8*)&gu8Status,2);
            break;
...
void hw_usb_endpoint_setup_handler(void)
{
    ...
    {        
        case GET_STATUS:
            ...
            hw_usb_ep_in_transfer(EP0,(uint8*)&u16Status,2);
            break;
...
//返  回: 无
//=========================================================================
void hw_usb_ep2_in_handler(uint8 *SendBuff,uint32 DataLenght)
{
    uint8 i,counter;
    uint8 *pBuffer;
    uint32 vEP2Idx = 0;
